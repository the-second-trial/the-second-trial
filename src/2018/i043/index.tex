...
The next part of the problem concerns a dimensioning task. The new requirements
for tiles can be formalized as follows:
\begin{equation}
\label{eq:fconds2}
\begin{cases}
f^\prime(0) = 0\\
...
\end{cases}
\end{equation}
Having $\gamma = \frac{55}{100}$ represent the fraction, $S_{\Lambda}$ represent the surface 
...
times the area below $f$ (hence $\Gamma$). Proving that $f_k$ (hence $\Gamma_k$) have the 
...
all copies of $f$ have the same area. 
The problem suggests the adoption of 2\textsuperscript{nd} or 3\textsuperscript{rd} 
order polynomials as drawing function $f$.

\begin{theorem}[2-degree polynomials aren't good]
\label{the:2degpoly}
